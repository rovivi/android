...
bool bitmapToMat(JNIEnv* env, jobject bmp, cv::Mat* out) {
  ...
  void* px = nullptr;
  if (AndroidBitmap_lockPixels(env, bmp, &px) < 0) return false;
  cv::Mat rgba(int(info.height), int(info.width), CV_8UC4, px);
...
Java_com_piu_ocr_PiuOcr_nativeRead(JNIEnv* env, jclass, jlong h, jobject bmp) {
  ...
  cv::Mat img;
  if (!c || !bitmapToMat(env, bmp, &img)) return env->NewStringUTF("{}");
